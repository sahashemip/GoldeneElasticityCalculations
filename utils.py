...
def quadratic_function(x, a, b):
    """Defines a quadratic function: f(x) = a*x^2 + b"""
    return a * x**2 + b

def read_data(filename):
    """Reads strain-energy data from a CSV file and returns strain and energy arrays."""
    try:
        data = np.genfromtxt(filename, delimiter=',', skip_header=1)
        if data.size == 0:
            raise ValueError("File '{}' is empty or improperly formatted.".format(filename))
        return data[:, 0], data[:, 1]  # Returns strain and energy values
    except IOError:
...
def plot_fit(strains, energies, fit_params):
    """Plots the original data and the fitted quadratic curve."""
    fit_y = quadratic_function(strains, *fit_params)
    plt.plot(strains, energies, 'o', label='Data')
    plt.plot(strains, fit_y, '-', label='Fitted Curve')
    plt.xlabel("Strain")
